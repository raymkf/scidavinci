...
export default defineConfig(({ mode }) => {
  ...
  const target = env.SCIDAVINCI_API_URL ?? env.NANOBOT_API_URL ?? "http://127.0.0.1:8765";
  ...
  return {
    ...
    server: {
      ...
      // the ``/`` proxy below (Vite HMR and the SciDaVinci ws upgrade both sit on
      ...
      proxy: {
        ...
        // Forward only WebSocket upgrades on ``/`` to the SciDaVinci gateway;
